...
| body_kind extension path | Present as a location rule, not as a local registry. DAGR body content uses the legacy compatibility path `extensions.garp.body` and is named by `body_kind`. | `docs/BODY_KIND_EXTENSION_RULES.md`; pack bodies under `extensions.garp.body.body_kind`. | This conformance repo defines no body-kind schema registry and blesses no DAGR body semantics. | Any future token-cost body must be registered upstream or through the existing body_kind / legacy `extensions.garp.body` path if that is the repo convention. |
